...
def test_duplicate_does_not_change_total(solution):
    ...
    assert total_for(totals, "m-verde", "2026-07-24T13:00:00+00:00") == 80_000
...
def test_deduplication_is_isolated_by_merchant(solution):
    shared_id_events = [
        {
            "event_id": "shared", "merchant_id": "m-a",
            "event_time": "2026-07-24T13:00:05Z", "arrival_time": "2026-07-24T13:00:06Z",
            "amount": 10, "status": "CONFIRMED",
        },
        {
            "event_id": "shared", "merchant_id": "m-b",
            "event_time": "2026-07-24T13:00:10Z", "arrival_time": "2026-07-24T13:00:11Z",
    ...
    assert {row["merchant_id"]: row["total"] for row in totals} == {"m-a": 10, "m-b": 20}
...
def test_stateful_dofn_keeps_keys_isolated(solution):
    ...
    with BeamTestPipeline() as pipeline:
        ...
        assert_that(output, equal_to([("m-a", "shared"), ("m-b", "shared")]))


def test_out_of_order_event_uses_its_event_time_window(solution):
    totals, _ = solution.summarize_payments(load_events())

    assert total_for(totals, "m-azul", "2026-07-24T13:00:00+00:00") == 170_000


def test_late_event_within_tolerance_is_a_revision(solution):
    _, audit = solution.summarize_payments(load_events(), allowed_lateness_seconds=180)
...
def test_event_beyond_lateness_is_audited(solution):
    _, audit = solution.summarize_payments(load_events(), allowed_lateness_seconds=120)
...
def test_windowed_pipeline_produces_totals(solution):
    events = [
        {
            "event_id": "p-1", "merchant_id": "m-a",
            "event_time": "2026-07-24T13:00:05Z", "arrival_time": "2026-07-24T13:00:06Z",
            "amount": 10, "status": "CONFIRMED",
        },
        {
            "event_id": "p-2", "merchant_id": "m-a",
            "event_time": "2026-07-24T13:00:42Z", "arrival_time": "2026-07-24T13:00:43Z",
            "amount": 20, "status": "CONFIRMED",
        },
    ]
...
def test_trigger_policy_has_lateness_and_accumulating_panes(solution):
    policy = solution.build_trigger_policy(window_seconds=60, allowed_lateness_seconds=120)
...
def test_append_only_sink_materializes_every_attempt(solution):
    ...
    materialized, audit = solution.simulate_sink_retries(results, attempts=2, idempotent=False)
